...
Faqs.push({
  ...
  Answer: (
    <Fragment>
      Payments can be made with credit card using Eventbrite via our tickets page when registrations are open. Companies
      that want to buy bulk tickets (> 5) can <a href={'mailto:' + Conference.ContactEmail}>contact us</a> to pay by
      invoice (EFT or credit card).
    </Fragment>
  ),
  Category: 'tickets',
})
...
Faqs.push({
  ...
  Answer: (
    <Fragment>
      We are always looking for volunteers and sometimes looking for organisers! It takes a lot of effort to organise a
      volunteer-run conference like {Conference.Name}. Shoot us an email at{' '}
      <a href={'mailto:' + Conference.ContactEmail}>{Conference.ContactEmail}</a> and we can work with you to figure out
...
Faqs.push({
  ...
  Answer: (
    <Fragment>
      ...
      recruiting opportunity based on being attended by people that don't normally get to go to conferences. It's also a
      great chance to give back and support the local software community. We have various levels of sponsorship
